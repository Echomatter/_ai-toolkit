...
for(const m of messages)for(const p of m.parts||[]){
 if(mode==='--restore'){
  if(p.state?.metadata?.ai_toolkit_delegate_display?.version!==1)continue;
  restoreDelegateTools([{parts:[p]}]);p.state.metadata={...p.state.metadata,ai_toolkit_display_disabled:true};await update(p);count++;
 }else{
  if(p.state?.metadata)delete p.state.metadata.ai_toolkit_display_disabled;
