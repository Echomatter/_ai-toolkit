...
test('live metadata uses authenticated part updates, preserves linkage and isolates simultaneous calls', async()=>{
 let saved=part();
 const client={session:{message:async()=>({data:{parts:[saved]}}),get:async()=>({data:{parentID:'parent',metadata:{ai_toolkit:{selected:'opencode/free'}}}})},part:{update:async args=>{saved=args.body;return{data:saved}}}};
 ...
 assert.match(saved.state.input.description,/read · model-evidence.json/);
