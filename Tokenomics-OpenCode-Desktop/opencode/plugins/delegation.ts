...
const DelegationPlugin: Plugin = async ({ client, directory }) => {
  ...
  const locator = path.join(base, 'opencode', 'tokenomics-root.txt')
    ...
    tool: {
      delegate: tool({
        ...
        args: {
          ...
          taskTypes: tool.schema.array(tool.schema.string()).optional().describe('Match the bounded deliverable: bounded_feature for named-file code work, research for source research, code_review for review. repo_navigation means finding/understanding an unfamiliar repository, not every file read. Only add requirements the assignment really needs.'),
