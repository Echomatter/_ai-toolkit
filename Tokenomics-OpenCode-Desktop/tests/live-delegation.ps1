...
$prompt='Read-only toolkit integration test. Invoke delegate exactly once with role worker, taskTypes [bounded_feature], needsWrites false, freeOnly true, preferredCostClass free, needsModelDiversity true. Task: Read opencode/catalog.json using read and return the exact five skill names. No shell, writes or nested agents. Return the delegate receipt. Do not retry or perform the task yourself.'
...
$calls=@($events|Where-Object{($_.part.tool-eq'delegate'-or$_.part.state.metadata.tokenomics_delegate_display.original_tool-eq'delegate')-and$_.part.state.status-eq'completed'}|Group-Object {$_.part.callID}|ForEach-Object{$_.Group[-1]})
...
if($receipt.status-ne'completed'){throw "Child status: $($receipt.status); failure: $($attempt.failure). Local log: $log"}
...
$displayParts=@($parent.messages|ForEach-Object{$_.parts}|Where-Object{$_.type-eq'tool'-and$_.state.metadata.tokenomics_delegate_display})
if($displayParts.Count-ne 1-or$displayParts[0].state.metadata.tokenomics_delegate_display.original_tool-ne'delegate'-or-not$displayParts[0].state.metadata.tokenomics_delegate_display.original_input.freeOnly){throw 'Completion lost the original delegate call or free-only input'}
Write-Output 'PASS: native display retains original delegate identity and inputs for model-history restoration'
