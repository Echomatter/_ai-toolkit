...
import { createDelegator, observe, failureKind, splitModel, freeOnlyAssignment } from '../tools/runtime/delegation.mjs';
...
test('freeOnly is forwarded, enforced at dispatch and inherited by descendants', async t => {
  ...
  assert.equal(g.sessions.get('child1').metadata.tokenomics.freeOnly, true);
  g.sessions.get('parent').metadata = { tokenomics: { selected: 'opencode/free-parent', readOnly: false, freeOnly: true } };
...
test('explicit no-write user assignment overrides a mistaken writer request', async t => {
  ...
  assert.equal(f.sessions.get('child1').metadata.tokenomics.readOnly, true);
