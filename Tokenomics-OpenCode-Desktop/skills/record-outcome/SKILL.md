...
Resolve the toolkit root from the installed `tokenomics-root.txt`. Call `scripts/record-task-outcome.ps1` with the real task ID returned by `delegate`, actual model, repo, task types and observed test/result values. Pass task types as one comma-separated string when using PowerShell `-File`.
