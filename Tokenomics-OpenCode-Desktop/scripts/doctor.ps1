...
if(Test-Path -LiteralPath $globalAgents){
  ...
  if($ga -match '<!-- BEGIN TOKENOMICS MANAGED BLOCK -->' -and $ga -match '<!-- END TOKENOMICS MANAGED BLOCK -->'){OK 'OpenCode global toolkit instruction block installed.'}else{WARN 'OpenCode global AGENTS.md exists but toolkit managed block was not found.'}
