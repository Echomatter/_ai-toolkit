...
export function createDelegator({ client, toolkitRoot, directory, select, record, beforeSelect = async () => {},
  ...
  async function lookup(id, directory) {
    ...
    while (id && !visited.has(id) && visited.size < 32) {
      ...
      const saved = session?.metadata?.tokenomics || session?.metadata?.ai_toolkit;
  ...
  async function run(args, ctx) {
    ...
    const freeOnly = inherited?.freeOnly === true || args.freeOnly === true || freeOnlyAssignment(assignment);
      ...
      for (let n = 0; n < 2; n++) {
        ...
        try {
          child = await call('session', 'create', { query: query(ctx.directory), body: {
            ...
            metadata: { tokenomics: { selected, readOnly, freeOnly } },
          } }, ctx.abort);
